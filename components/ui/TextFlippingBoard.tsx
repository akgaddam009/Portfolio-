  ...
  return (
    <div className="flex aspect-[3/6] flex-col overflow-hidden rounded-[2px] border border-neutral-300 md:rounded-[3px] md:border-2 dark:border-black">
      <div className="relative flex-1 [perspective:100px] [transform-style:preserve-3d]">
        ...
        {/* Flipping top flap — old char drops */}
        {flipId > 0 && (
          <motion.div
            key={flipId}
            className={cn(
        ...
        {/* Flipping bottom flap — new char rises */}
        {flipId > 0 && (
          <motion.div
            key={`b${flipId}`}
            className={cn(
      ...
      <div className="h-2 w-full opacity-20 md:h-4 bg-[repeating-linear-gradient(to_bottom,currentColor_0,currentColor_1px,transparent_1px,transparent_0.2rem)] text-neutral-400 dark:text-black dark:opacity-100" />
    </div>
  );
},
(p, n) =>
...
export function TextFlippingBoard({
  ...
  return (
    <div
      className={cn(
        "relative mx-auto w-full max-w-3xl rounded-xl bg-neutral-100 p-2 shadow-xl",
        "md:rounded-2xl md:p-4",
        "dark:bg-neutral-900 dark:shadow-[0_20px_70px_-15px_rgba(0,0,0,0.6)]",
        className,
      )}
    >
      <div
